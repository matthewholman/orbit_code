...
void 
mrqmin_orbit(struct reb_simulation* sim,
	     struct assist_ephem* ephem,
	     OBSERVATION obsarray[], int ndata, double a[], int ia[],
	     int ma, double **covar, double **alpha, double *chisq,
	     double *alamda, double energy_wt);

void
mrqfit(struct reb_simulation* sim,
       struct assist_ephem* ephem,
...
int
fit_observations(struct reb_simulation* sim,
		 struct assist_ephem* ephem,
		 OBSERVATION obsarray[],
		 int nobs,
		 PBASIS *p,
		 ...
		 FILE *logfile)
{
    ...
    if (logfile!=NULL) {
      ...
      fprintf(logfile,"#  %8le %8le %8le %8le %8le %8le\n",p->a,p->adot,p->b,
     p->bdot, p->g, p->gdot);  
    }
	*/
  ...
  if (fitparms==6) {
    ...
    mrqfit(sim, ephem, obsarray, nobs, p, ia, covar, chisq, energy_fit);
     ...
     */
    {
      ...
      gbind2 = 2*GM0*pow(p->g,3.)/sqrt(t) - p->adot*p->adot - p->bdot*p->bdot;
      /* Allow a 2-sigma excursion: */
      var_gbind2 = 36*GM0*GM0*pow(p->g,4.)/t*covar[5][5] +
 ...
 //fitparms=6;	
  ...
  if (fitparms<6) {
      ...
      if (logfile!=NULL) 
	  fprintf(logfile, "#WARNING: Fitting with energy constraint.  fitpars=%d\n", fitparms);
	  ...
	  p->g = 0.03;  // This should be updated to be flexible.
  ...
  if (fitparms==4) {
    ...
    covar[6][6] = GM0*pow(p->g,3.)/SIGMA2UNBOUND;
     for (i=1; i<=5; i++) 
...
/* Return value is 5 if energy constraint is used, 6 otherwise */
int
fit_observations_prelim(struct reb_simulation* sim,
		 struct assist_ephem* ephem,
		 OBSERVATION obsarray[],
		 int nobs,
		 PBASIS *p,
       ...
       double energy_wt)
{
   ...
   *chisq = 1.e14;
   ...
   do {
     ...
     mrqmin_orbit(sim, ephem, obsarray, nobs, a, ia,
