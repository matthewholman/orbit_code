...
void mrqmin_orbit(struct reb_simulation* sim,
		  ...
		  double *alamda, double energy_wt)
{
	...
	if (*alamda < 0.0) {
		...
		*alamda=0.001;
		mrqcof_orbit(sim, ephem, obsarray,ndata,a,ia,ma,alpha,beta,chisq,
			     energy_wt);
		ochisq=(*chisq);
	...
	mrqcof_orbit(sim, ephem, obsarray,ndata,atry,ia,ma,covar,da,chisq,
		     energy_wt);
	if (*chisq < ochisq) {
		*alamda *= 0.1;
		ochisq=(*chisq);
